module SuccessAndFailure.Validation4 where
...
main :: IO ()
main = do
    pass <- prompt "Please enter a password." >> (Password <$> getPass)
...
validatePassword (Password pass) = cleanWhitespace pass >>= requireAlpha >>= checkPasswordLength
